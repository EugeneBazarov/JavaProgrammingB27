...
public class PrimeInRange {
    public static void main(String[] args) {
        ...
        print all the prime numbers from 2 to 50
        2 3 5 7 11 .... 47
     ...
     */
        ...
        for(int i = 2; i <= range; i++){ // starting from 2 and going until the range number
            ...
            for(int j = 2; j < i; j++) { // starting from 2 because I want to check divisibility of the number from 2 to the number itself
                ...
                if(i % j == 0){ // the number we are checking (i) is evenly divisible by the j values (from 2 to the number) it means it's not prime
                    isPrime = false;
