...
Example:
    ...
    Output:
        ...
        for (int i = nums.length - 1; i >= 0; i--) { // read th numbers from the end, we sorted sp I will keep running until I find a new number that is not the same as the max number
            if (nums[i] != max) {
